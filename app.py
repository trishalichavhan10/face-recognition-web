...
def student_profile(student_id):
    ...
    c.execute("SELECT COUNT(DISTINCT DATE(timestamp)) FROM attendance WHERE student_id=?", (student_id,))
...
@app.route("/student_image/<int:student_id>")
def student_image(student_id):
    folder = os.path.join(DATASET_DIR, str(student_id))
    for fname in ["0.jpg", "1.jpg", "2.jpg"]:
...
def dashboard_stats():
    ...
    conn.close()
    return jsonify({"total": total, "today": today_count, "classes": total_classes})
